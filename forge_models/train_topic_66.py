#!/usr/bin/env python3
"""Topic 66 — 7d SOL/USD log-return: train / evaluate / export predict.pkl.
...
Horizon mapping (shared with topics 68/70):
...
Data source note: prefer DATA_SOURCE=allora (Tiingo via Atlas). SOL is a
volatile alt — confirm Tiingo history depth on the first backfill.

    export ALLORA_API_KEY=UP-...
    python forge_models/train_topic_66.py
    # -> writes models/predict_topic_66.pkl

Deploy:
    PREDICT_PKL=models/predict_topic_66.pkl TOPIC_ID=66 \
...
# 7d SOL moves can be very large; this guard only warns (never clips).
...
def main() -> None:
    ...
    def predict(nonce: int | None = None) -> float:
        """Return the predicted 7d SOL/USD **log-return** (not a price).
