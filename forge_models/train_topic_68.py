#!/usr/bin/env python3
"""Topic 68 — 7d ETH/USD log-return: train / evaluate / export predict.pkl.

Same task family as the BTC/ETH log-return topics, but at a **7-day horizon**
with **hourly updates**. Target: log_return = ln(price_{t+7d} / price_t). The
exported ``predict(nonce)`` returns the predicted **log-return as a float**.

Horizon mapping (the real difference from the 1h/8h topics):
    interval="1h", target_bars=168          # 168 hours = 7 days
We keep the *base resolution at 1h* (not 1d or 7d bars) on purpose: the topic
updates every hour, so live features — and therefore submissions — must refresh
hourly. The target is simply the 168-bars-ahead log return (verified against the
kit's compute_target_polars: target = log(close.shift(-168)) - log(close)).

Two adaptations follow from the long horizon, both kept inside this script so
``forge_models/common`` stays untouched:

* Vol scale. ``compact_features`` returns ``sigma`` = per-BAR (1h) return std.
  The 7d return is ~sqrt(168) larger, so we vol-normalize the target by
  ``sigma_h = sigma * sqrt(target_bars)`` instead of the raw per-bar sigma —
  otherwise z = r_7d / sigma_1h ~ N(0, ~13) and Z_CLIP would shred it. (For the
  1h/8h topics target_bars=1, so sqrt(1)=1 and this reduces to their behavior.)
* Feature reach. The default feature geometry tops out at 96 bars (4 days),
  too short-sighted for a 7d forecast. We pass longer ``ret_lags``/``vol_windows``
  (up to 14 days) — keeping 6/24/96 present so the rv_ratio block stays valid —
  and grow INPUT_BARS to fit. compact_features is called with the SAME kwargs in
  training and in predict(), so the feature set matches.

Caveat — effective sample size: hourly samples of a 7d return overlap heavily
(consecutive rows share 167/168 of their window), so the *effective* number of
independent observations is ~ calendar_days / 7, far below the row count. We
lean on shallow trees, recency weighting, and lambda-shrink; the walk-forward
folds use gap=target_bars so test targets never overlap train features.

Data source note: prefer DATA_SOURCE=allora (Tiingo via Atlas, resampled from
1-minute) — it matches scoring and handles 1h cleanly. The Binance fallback is a
no-API-key escape hatch only.

    export ALLORA_API_KEY=UP-...
    python forge_models/train_topic_68.py
    # -> writes models/predict_topic_68.pkl

Deploy:
    PREDICT_PKL=models/predict_topic_68.pkl TOPIC_ID=68 \
...
# 7d ETH moves can be large; this guard only warns (never clips).
...
def main() -> None:
    ...
    def predict(nonce: int | None = None) -> float:
        """Return the predicted 7d ETH/USD **log-return** (not a price).
