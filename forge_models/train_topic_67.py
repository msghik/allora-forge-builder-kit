#!/usr/bin/env python3
"""Topic 67 — 7d BTC/USD log-return: train / evaluate / export predict.pkl.
...
Horizon mapping (shared with topics 66/68/70):
...
Data source note: prefer DATA_SOURCE=allora (Tiingo via Atlas). BTC has the
deepest, cleanest history of the set, so 1400d should backfill comfortably.

    export ALLORA_API_KEY=UP-...
    python forge_models/train_topic_67.py
    # -> writes models/predict_topic_67.pkl

Deploy:
    PREDICT_PKL=models/predict_topic_67.pkl TOPIC_ID=67 \
...
# BTC is the calmest of the set; this guard only warns (never clips).
MAX_PLAUSIBLE_LOGRET = 0.6
...
def main() -> None:
    ...
    def predict(nonce: int | None = None) -> float:
        """Return the predicted 7d BTC/USD **log-return** (not a price).
