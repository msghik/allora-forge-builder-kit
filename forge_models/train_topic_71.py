#!/usr/bin/env python3
"""Topic 71 — 8h NEAR/USD log-return: train / evaluate / export predict.pkl.

Same task shape as topic 83 (8h BTC/USD log-return) — only the asset differs.
Target: log_return = ln(price_{t+8h} / price_t). The exported ``predict(nonce)``
returns the predicted **log-return as a float** — for log-return topics you
submit the log-return itself, NOT a price.

Horizon mapping (shared with topic 83):
    interval="8h", target_bars=1
The bar IS the 8h horizon, so ``sigma100`` (trailing std of the last 100 per-bar
log-returns) equals the competition's ZPTAE reference std, which here is over
the last 100 non-overlapping 8h windows. All of ``forge_models/common``
(features, vol-normalization, calibration, export) is reused unchanged.
...
hatch and is unreliable at this horizon.
    ...
    python forge_models/train_topic_71.py
    # -> writes models/predict_topic_71.pkl

Deploy:
    PREDICT_PKL=models/predict_topic_71.pkl TOPIC_ID=71 \
...
# NEAR is a more volatile alt than BTC; 8h moves can be large. This guard only
# prints a warning (it does not clip), so keep it loose to avoid false alarms.
MAX_PLAUSIBLE_LOGRET = 0.5

# --- windowing / history ---
INPUT_BARS = int(os.environ.get("INPUT_BARS", "128"))   # >=101 so sigma100 fits
# 8h bars are ~3/day; pull more calendar history for sample count.
...
def main() -> None:
    ...
    def predict(nonce: int | None = None) -> float:
        """Return the predicted 8h NEAR/USD **log-return** (not a price).
